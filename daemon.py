...
PLAYLIST_DIR = STEAMCAST_DIR / "playlists"
DEFAULT_PORT = 6789
# Concat playlist entries per stream: enough copies of the video to cover
# ~96h of continuous looping. Playlists are regenerated by the daemon, so
# this only needs to outlive one daemon session + schedule window.
PLAYLIST_COVER_HOURS = 96
PLAYLIST_MIN_ENTRIES = 16
...
class DaemonManager:
    ...
    def _ensure_playlist(self, video: str) -> Optional[str]:
        """Write a concat playlist looping `video` for ~PLAYLIST_COVER_HOURS.

        Returns the playlist path, or None on failure. Using the concat
        demuxer instead of ``-stream_loop -1`` because ffmpeg's stream_loop
        restart corrupts DTS when combined with ``-ss`` input seeking —
        every loop wrap sent timestamps negative and Steam's ingest dropped
        the connection (exit 152), fragmenting upload sessions.

        The concat demuxer restarts each listed file cleanly: output
        timestamps stay monotonic across the wrap, so a resume offset keeps
        working and the stream survives indefinitely.
        """
        try:
            PLAYLIST_DIR.mkdir(parents=True, exist_ok=True)
            dur = self._probe_duration(str(video))
            if not dur or dur <= 0:
                dur = 3600.0  # fallback: assume 1h, min entries still applies
            n = max(PLAYLIST_MIN_ENTRIES, int(PLAYLIST_COVER_HOURS * 3600 / dur) + 1)
            n = min(n, 5000)  # sanity cap — one file per line, tiny either way
            video_abs = str(Path(video).resolve())
            # concat demuxer: single-quote paths (spaces in filenames like
            # "dreadout 2.mp4" are fine inside quotes)
            lines = "".join(f"file '{video_abs}'\n" for _ in range(n))
            # Stable filename across restarts (hash() is salted per process)
            digest = hashlib.md5(video_abs.encode()).hexdigest()[:12]
            path = PLAYLIST_DIR / f"stream_{digest}.txt"
            if not path.exists() or path.read_text() != lines:
                path.write_text(lines)
            return str(path)
        except Exception as exc:
            logger.warning("Could not build concat playlist for %s: %s", video, exc)
            return None
    ...
    def _build_stream_args(self, ffmpeg: str, video: str, stream_key: str,
        ...
        The video is looped via a concat playlist (replacing -stream_loop -1,
        which corrupts DTS at the wrap when combined with -ss — see
        _ensure_playlist). A resume offset past the end wraps via modulo when
        duration is known — playback continues seamlessly instead of
        restarting at 00:00 on every reconnect/daemon restart.
        """
        playlist = self._ensure_playlist(video)
        if not playlist:
            return []
        args = [ffmpeg, "-re"]
        if resume_offset > 0:
    ...
    def _ffmpeg_stderr_path(self, gname: str) -> str:
        """Per-stream ffmpeg stderr file so deaths are diagnosable (stderr
        used to be DEVNULL — exit 152 was a blind number). Rotated on each
        spawn via 'w' mode."""
        try:
            STEAMCAST_DIR.mkdir(parents=True, exist_ok=True)
            safe = re.sub(r"[^A-Za-z0-9_-]", "_", gname)
            return str(STEAMCAST_DIR / f"ffmpeg_{safe}.log")
        except Exception:
            return os.devnull
    ...
    def _spawn_stream(self, args: list[str], gname: str):
        ...
        err_fh.close()  # fd stays open in the child
        return proc
    ...
    def _run_engine(self, games: list[dict], restart_every: int):
        ...
        for game in games:
            ...
            proc = self._spawn_stream(args, gname)
            ...
            with self._streams_lock:
                for gname, stream in list(self._active_streams.items()):
                    ...
                    if proc and proc.poll() is not None:
                        ...
                        if args:
                            new_proc = self._spawn_stream(args, gname)
